...
4. 'topic': The specific topic or chapter name(s). IMPORTANT: PracUp explicitly supports generating a worksheet for SINGLE or MULTIPLE topics/chapters simultaneously (e.g. "Body Movements, Fun with Magnets" or "Fractions, Decimals"). NEVER tell the user that the system only supports one topic per worksheet.
...
- Multiple topics: If the user selects or mentions multiple topics/chapters, join them as a comma-separated string (e.g. "Body Movements, Fun with Magnets") in the 'topic' field. Confirm to the user that you have included all requested topics!
...
SCHEMA:
{
  ...
  "params": {
    ...
    "topic": "extracted topic string (can be comma-separated for multiple topics)",
    ...
    if (aiResult && aiResult.params) {
      ...
      if (grade && !validGrades.includes(grade)) {
        ...
        if (availableTopics.length === 0) {
          ...
          // Split topic into individual requested topic strings to support multiple topics
          const rawTopicParts = (Array.isArray(topic) ? topic : String(topic).split(/[,+]/)).map(t => t.trim()).filter(Boolean);
          const matchedTopics: string[] = [];

          for (const part of rawTopicParts) {
            const normalizedPart = part.toLowerCase().replace(/[^a-z0-9]/g, "");
            const matched = availableTopics.find(t => {
            ...
            if (matched) {
              if (!matchedTopics.includes(matched.name)) {
                matchedTopics.push(matched.name);
              }
            } else {
              // If no exact fuzzy match, retain the user's explicit part name
              if (!matchedTopics.includes(part)) {
                matchedTopics.push(part);
              }
            }
          }

          if (matchedTopics.length > 0) {
            aiResult.params.topic = matchedTopics.join(", ");
            ...
            aiResult.clarifyingMessage = `I couldn't find chapters matching "${topic}" in the CBSE ${grade} ${subjectUpper} syllabus. Please choose from the available topics:\n${availableTopics.map(t => `• ${t.name}`).join("\n")}`;
            aiResult.params = null;
            aiResult.isComplete = false;
          }
        }
      ...
      if (aiResult.isComplete) {
        if (!aiResult.params || !aiResult.params.grade || !aiResult.params.subject || !aiResult.params.topic) {
          aiResult.isComplete = false;
          aiResult.params = null;
          aiResult.clarifyingMessage = `Please make sure you specify the grade, subject, and topic(s) for the worksheet.`;
        } else {
